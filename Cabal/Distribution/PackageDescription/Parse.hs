...
readAndParseFile :: (FilePath -> (String -> CabalM a) -> CabalM a)
                 -> (String -> ParseResult a)
                 -> FilePath -> CabalM a
readAndParseFile withFileContents' parser fpath = do
  ...
  withFileContents' fpath $ \str -> case parser str of
    ...
    ParseOk warnings x -> do
        traverse_ (warn . showPWarning fpath) $ reverse warnings
        return x

readHookedBuildInfo :: FilePath -> CabalM HookedBuildInfo
readHookedBuildInfo =
    readAndParseFile (\fp act -> runCabalMInIO $ \liftC -> withFileContents fp (liftC . act)) parseHookedBuildInfo

readPackageDescription :: FilePath -> CabalM GenericPackageDescription
...
readGenericPackageDescription =
    readAndParseFile (\fp act -> runCabalMInIO $ \liftC -> withUTF8FileContents fp (liftC . act)) parseGenericPackageDescription
