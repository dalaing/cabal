...
register pkg_descr lbi0 flags =
   ...
   doRegister
 where
  doRegister = flip runCabalM verbosity $ do
    targets <- readTargetInfos pkg_descr lbi0 (regArgs flags)
    ...
    liftIO $ registerAll pkg_descr lbi0 flags (catMaybes ipi_mbs)
   where
...
generateOne pkg lib lbi clbi regFlags
  = flip runCabalM verbosity $ do
    absPackageDBs    <- liftIO $ absolutePackageDBPaths packageDbs
    installedPkgInfo <- generateRegistrationInfo
                           pkg lib lbi clbi inplace reloc distPref
                           (registrationPackageDB absPackageDBs)
    info (IPI.showInstalledPackageInfo installedPkgInfo)
...
registerAll pkg lbi regFlags ipis
  = flip runCabalM verbosity $ do
    when (fromFlag (regPrintId regFlags)) $ do
      for_ ipis $ \installedPkgInfo ->
        ...
        when (packageId installedPkgInfo == packageId pkg
          ...
          liftIO $ putStrLn (display (IPI.installedUnitId installedPkgInfo))
    ...
    case () of
     _ | modeGenerateRegFile   -> writeRegistrationFileOrDirectory
       ...
       | otherwise             -> do
           for_ ipis $ \ipi -> do
               setupMessage' "Registering" (packageId pkg)
  ...
  where
    ...
    writeRegistrationFileOrDirectory = do
      ...
      liftIO $ deletePackageDB regFile
      case ipis of
        [installedPkgInfo] -> do
          info ("Creating package registration file: " ++ regFile)
          liftIO $ writeUTF8File regFile (IPI.showInstalledPackageInfo installedPkgInfo)
        _ -> do
          info ("Creating package registration directory: " ++ regFile)
          liftIO $ createDirectory regFile
          let num_ipis = length ipis
              lpad m xs = replicate (m - length ys) '0' ++ ys
                  where ys = take m xs
              number i = lpad (length (show num_ipis)) (show i)
          liftIO $ for_ (zip ([1..] :: [Int]) ipis) $ \(i, installedPkgInfo) ->
    ...
    writeRegisterScript =
      case compilerFlavor (compiler lbi) of
        JHC -> notice "Registration scripts not needed for jhc"
        UHC -> notice "Registration scripts not needed for uhc"
        _   -> withHcPkg
               "Registration scripts are not implemented for this compiler"
               (compiler lbi) (withPrograms lbi)
               (writeHcPkgRegisterScript ipis packageDbs)
...
generateRegistrationInfo pkg lib lbi clbi inplace reloc distPref packageDb = do
  --TODO: eliminate pwd!
  ...
  installedPkgInfo <-
    if inplace
      ...
      then return (inplaceInstalledPackageInfo pwd distPref
        ...
        abi_hash <- abiHash pkg distPref lbi lib clbi
        if reloc
          then relocRegistrationInfo
...
abiHash :: PackageDescription
        -> FilePath
        -> LocalBuildInfo
        -> Library
        -> ComponentLocalBuildInfo
        -> CabalM AbiHash
abiHash pkg distPref lbi lib clbi =
    case compilerFlavor comp of
     GHC | compilerVersion comp >= mkVersion [6,11] -> do
            fmap mkAbiHash $ GHC.libAbiHash pkg lbi' lib clbi
     GHCJS -> do
            fmap mkAbiHash $ GHCJS.libAbiHash pkg lbi' lib clbi
     _ -> return (mkAbiHash "")
  where
    comp = compiler lbi
    lbi' = lbi {
...
relocRegistrationInfo :: PackageDescription
                      -> Library
                      -> LocalBuildInfo
                      -> ComponentLocalBuildInfo
                      -> AbiHash
                      -> PackageDB
                      -> CabalM InstalledPackageInfo
relocRegistrationInfo pkg lib lbi clbi abi_hash packageDb =
  case (compilerFlavor (compiler lbi)) of
    GHC -> do fs <- GHC.pkgRoot lbi packageDb
...
initPackageDB :: Compiler -> ProgramDb -> FilePath -> CabalM ()
initPackageDB comp progdb dbPath =
    createPackageDB comp progdb False dbPath

-- | Create an empty package DB at the specified location.
createPackageDB :: Compiler -> ProgramDb -> Bool
                -> FilePath -> CabalM ()
createPackageDB comp progdb preferCompat dbPath =
    case compilerFlavor comp of
      GHC   -> HcPkg.init (GHC.hcPkgInfo   progdb) preferCompat dbPath
      GHCJS -> HcPkg.init (GHCJS.hcPkgInfo progdb) False dbPath
      LHC   -> HcPkg.init (LHC.hcPkgInfo   progdb) False dbPath
      UHC   -> return ()
      HaskellSuite _ -> HaskellSuite.initPackageDB progdb dbPath
      _              -> die' $
...
invokeHcPkg :: Compiler -> ProgramDb -> PackageDBStack
                -> [String] -> CabalM ()
invokeHcPkg comp progdb dbStack extraArgs =
  withHcPkg "invokeHcPkg" comp progdb
    (\hpi -> HcPkg.invoke hpi dbStack extraArgs)

withHcPkg :: String -> Compiler -> ProgramDb
          -> (HcPkg.HcPkgInfo -> CabalM a) -> CabalM a
...
registerPackage :: Compiler
                -> ProgramDb
                -> PackageDBStack
                -> InstalledPackageInfo
                -> HcPkg.RegisterOptions
                -> CabalM ()
registerPackage comp progdb packageDbs installedPkgInfo registerOptions =
  case compilerFlavor comp of
    GHC   -> GHC.registerPackage   progdb packageDbs installedPkgInfo registerOptions
    GHCJS -> GHCJS.registerPackage progdb packageDbs installedPkgInfo registerOptions
    ...
    LHC   -> LHC.registerPackage   progdb packageDbs installedPkgInfo registerOptions
    UHC   -> UHC.registerPackage   comp progdb packageDbs installedPkgInfo
    JHC   -> notice "Registering for jhc (nothing to do)"
    HaskellSuite {} ->
      HaskellSuite.registerPackage progdb packageDbs installedPkgInfo
    _    -> die' "Registering is not implemented for this compiler"

writeHcPkgRegisterScript :: [InstalledPackageInfo]
                         -> PackageDBStack
                         -> HcPkg.HcPkgInfo
                         -> CabalM ()
writeHcPkgRegisterScript ipis packageDbs hpi = do
  ...
  info ("Creating package registration script: " ++ regScriptFileName)
  liftIO $ do
...
unregister pkg lbi regFlags = flip runCabalM verbosity $ do
  let pkgid     = packageId pkg
      ...
      unreg hpi =
        ...
        in if genScript
             then liftIO $ writeFileAtomic unregScriptFileName
                    (BS.Char8.pack $ invocationAsSystemScript buildOS invocation)
             else runProgramInvocation invocation
  setupMessage "Unregistering" pkgid
