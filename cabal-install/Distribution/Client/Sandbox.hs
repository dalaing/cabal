...
import Distribution.Monad                     ( CabalM, runCabalM, runCabalMInIO, liftIO, askVerbosity )
...
tryLoadSandboxConfig :: GlobalFlags
                        -> CabalM (FilePath, PackageEnvironment)
tryLoadSandboxConfig globalFlags = do
  path <- liftIO $ getSandboxConfigFilePath globalFlags
  tryLoadSandboxPackageEnvironmentFile path
    (globalConfigFile globalFlags)

-- | Return the name of the package index file for this package environment.
tryGetIndexFilePath :: SavedConfig -> CabalM FilePath
...
-- 'SavedConfig'.
tryGetIndexFilePath' :: GlobalFlags -> CabalM FilePath
...
getSandboxPackageDB :: ConfigFlags -> CabalM PackageDB
...
getInstalledPackagesInSandbox ::    ConfigFlags
                                 -> Compiler -> ProgramDb
                                 -> CabalM InstalledPackageIndex
getInstalledPackagesInSandbox configFlags comp progdb = do
    sandboxDB <- getSandboxPackageDB configFlags
    getPackageDBContents comp sandboxDB progdb

-- | Temporarily add $SANDBOX_DIR/bin to $PATH.
withSandboxBinDirOnSearchPath :: FilePath -> CabalM a -> CabalM a
withSandboxBinDirOnSearchPath sandboxDir act =
    runCabalMInIO $ \liftC -> bracket_ addBinDir rmBinDir (liftC act)
  where
...
initPackageDBIfNeeded :: ConfigFlags
                         -> Compiler -> ProgramDb
                         -> CabalM ()
initPackageDBIfNeeded configFlags comp progdb = do
  SpecificPackageDB dbPath <- getSandboxPackageDB configFlags
  packageDBExists <- liftIO $ doesDirectoryExist dbPath
  unless packageDBExists $
    Register.initPackageDB comp progdb dbPath
  when packageDBExists $
    debug $ "The package database already exists: " ++ dbPath

-- | Entry point for the 'cabal sandbox dump-pkgenv' command.
dumpPackageEnvironment :: SandboxFlags -> GlobalFlags -> CabalM ()
dumpPackageEnvironment _sandboxFlags globalFlags = do
  (sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags
  commentPkgEnv        <- liftIO $ commentPackageEnvironment sandboxDir
  liftIO . putStrLn . showPackageEnvironmentWithComments (Just commentPkgEnv) $ pkgEnv

-- | Entry point for the 'cabal sandbox init' command.
sandboxInit :: SandboxFlags  -> GlobalFlags -> CabalM ()
sandboxInit sandboxFlags globalFlags = do
  -- Warn if there's a 'cabal-dev' sandbox.
  isCabalDevSandbox <- liftIO $ liftM2 (&&) (doesDirectoryExist "cabal-dev")
  ...
  createDirectoryIfMissingVerbose True sandboxDir'
  sandboxDir <- liftIO $ tryCanonicalizePath sandboxDir'
  liftIO $ setFileHidden sandboxDir

  -- Determine which compiler to use (using the value from ~/.cabal/config).
  userConfig <- loadConfig (globalConfigFile globalFlags)
  (comp, platform, progdb) <- liftIO $ configCompilerAuxEx (savedConfigureFlags userConfig)

  -- Create the package environment file.
  pkgEnvFile <- liftIO $ getSandboxConfigFilePath globalFlags
  createPackageEnvironmentFile sandboxDir pkgEnvFile comp platform
  (_sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags
  ...
  if indexFileExists
    then notice $ "Using an existing sandbox located at " ++ sandboxDir
    else notice $ "Creating a new sandbox at " ++ sandboxDir
  Index.createEmpty indexFile
...
sandboxDelete :: SandboxFlags -> GlobalFlags -> CabalM ()
sandboxDelete _sandboxFlags globalFlags = do
  (useSandbox, _) <- loadConfigOrSandboxConfig
                       globalFlags { globalRequireSandbox = Flag False }
  case useSandbox of
    NoSandbox -> warn "Not in a sandbox."
    UseSandbox sandboxDir -> do
      ...
      if isNonDefaultConfigLocation
        ...
        else liftIO $ removeFile pkgEnvFile
      ...
      absSandboxDir <- liftIO $ canonicalizePath sandboxDir
      notice $ "Deleting the sandbox located at " ++ absSandboxDir
      liftIO $ removeDirectoryRecursive absSandboxDir
        ...
        checkPackagePaths var = do
          let
            checkPath path = do
              ...
              (when (pathInsideSandbox absPath) . warn)
                (var ++ " refers to package database " ++ path
                 ++ " inside the deleted sandbox.")
          liftM (maybe [] splitSearchPath) (liftIO $ lookupEnv var) >>= mapM_ checkPath
...
doAddSource :: [FilePath] -> FilePath -> PackageEnvironment
               -> BuildTreeRefType
               -> CabalM ()
doAddSource buildTreeRefs sandboxDir pkgEnv refType = do
  ...
  (comp, platform, _) <- liftIO . configCompilerAuxEx . savedConfigureFlags $ savedConfig
  maybeAddCompilerTimestampRecord sandboxDir indexFile
    (compilerId comp) platform
  ...
  withAddTimestamps sandboxDir $ do
    ...
    buildTreeRefs' <- liftIO $ mapM tryCanonicalizePath buildTreeRefs
    Index.addBuildTreeRefs indexFile buildTreeRefs' refType
    return buildTreeRefs'

-- | Entry point for the 'cabal sandbox add-source' command.
sandboxAddSource :: [FilePath] -> SandboxFlags -> GlobalFlags
                    -> CabalM ()
sandboxAddSource buildTreeRefs sandboxFlags globalFlags = do
  (sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags

  if fromFlagOrDefault False (sandboxSnapshot sandboxFlags)
    then sandboxAddSourceSnapshot buildTreeRefs sandboxDir pkgEnv
    else doAddSource buildTreeRefs sandboxDir pkgEnv LinkRef

-- | Entry point for the 'cabal sandbox add-source --snapshot' command.
sandboxAddSourceSnapshot ::    [FilePath] -> FilePath
                            -> PackageEnvironment
                            -> CabalM ()
sandboxAddSourceSnapshot buildTreeRefs sandboxDir pkgEnv = do
  let snapshotDir = sandboxDir </> snapshotDirectoryName

  -- Use 'D.S.SrcDist.prepareTree' to copy each package's files to our private
  -- location.
  createDirectoryIfMissingVerbose True snapshotDir
  ...
  tmpDirs <- forM (zip buildTreeRefs pkgs) $ \(buildTreeRef, pkg) ->
    inDir (Just buildTreeRef) $ do
      ...
      prepareTree pkg Nothing targetTmpDir knownSuffixHandlers
      return (targetTmpDir, targetDir)

  -- Now rename the "snapshots/$PKGNAME-$VERSION-tmp" dirs to
  -- "snapshots/$PKGNAME-$VERSION".
  snapshots <- liftIO . forM tmpDirs $ \(targetTmpDir, targetDir) -> do
  ...
  doAddSource snapshots sandboxDir pkgEnv SnapshotRef

-- | Entry point for the 'cabal sandbox delete-source' command.
sandboxDeleteSource :: [FilePath] -> SandboxFlags -> GlobalFlags
                       -> CabalM ()
sandboxDeleteSource buildTreeRefs _sandboxFlags globalFlags = do
  (sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags
  indexFile            <- tryGetIndexFilePath (pkgEnvSavedConfig pkgEnv)

  (results, convDict) <-
    Index.removeBuildTreeRefs indexFile buildTreeRefs
  ...
  unless (null removedPaths) $ do
    ...
    notice $ "Success deleting sources: " ++
      showL removedRefs ++ "\n\n"
  ...
  notice $ "Note: 'sandbox delete-source' only unregisters the " ++
  ...
  where
    ...
    handleErrors errs@(Index.ErrNonregisteredSource{}:_) =
      warn ("Sources not registered: " ++ showE errs ++ "\n\n")
...
sandboxListSources :: SandboxFlags -> GlobalFlags
                      -> CabalM ()
sandboxListSources _sandboxFlags globalFlags = do
  (sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags
  indexFile            <- tryGetIndexFilePath (pkgEnvSavedConfig pkgEnv)

  refs <- Index.listBuildTreeRefs
          Index.ListIgnored Index.LinksAndSnapshots indexFile
  when (null refs) $
    notice $ "Index file '" ++ indexFile
    ++ "' has no references to local build trees."
  when (not . null $ refs) $ do
    notice $ "Source dependencies registered "
      ++ "in the current sandbox ('" ++ sandboxDir ++ "'):\n\n"
    liftIO $ mapM_ putStrLn refs
...
sandboxHcPkg :: SandboxFlags -> GlobalFlags -> [String] -> CabalM ()
sandboxHcPkg _sandboxFlags globalFlags extraArgs = do
  (sandboxDir, pkgEnv) <- tryLoadSandboxConfig globalFlags
  ...
  Register.invokeHcPkg comp progdb dbStack extraArgs
...
loadConfigOrSandboxConfig :: GlobalFlags  -- ^ For @--config-file@ and
                                          -- @--sandbox-config-file@.
                          -> CabalM (UseSandbox, SavedConfig)
loadConfigOrSandboxConfig globalFlags = do
  ...
  pkgEnvDir  <- liftIO $ getPkgEnvDir sandboxConfigFileFlag
  pkgEnvType <- liftIO $ classifyPackageEnvironment pkgEnvDir sandboxConfigFileFlag
                                           ignoreSandboxFlag
  case pkgEnvType of
    ...
    UserPackageEnvironment    -> do
      config <- loadConfig configFileFlag
      userConfig <- loadUserConfig pkgEnvDir Nothing
    ...
    AmbientPackageEnvironment -> do
      ...
      globalConstraintConfig <-
        loadUserConfig pkgEnvDir globalConstraintsOpt
  ...
  where
    ...
    dieIfSandboxRequired :: SavedConfig -> CabalM ()
...
-- nothing.
maybeWithSandboxDirOnSearchPath :: UseSandbox -> CabalM a -> CabalM a
...
reinstallAddSourceDeps ::    ConfigFlags  -> ConfigExFlags
                          -> InstallFlags -> GlobalFlags
                          -> FilePath
                          -> CabalM WereDepsReinstalled
reinstallAddSourceDeps configFlags' configExFlags
  ...
  retVal                   <- liftIO $ newIORef NoDepsReinstalled

  withSandboxPackageInfo configFlags globalFlags
    ...
    unless (null $ modifiedAddSourceDependencies sandboxPkgInfo) $ do
      ...
      withRepoContext globalFlags $ \repoContext -> do
        ...
        withSandboxBinDirOnSearchPath sandboxDir $ do
          installContext <- makeInstallContext args Nothing
          installPlan    <- foldProgress logMsg die'' return =<<
                            makeInstallPlan args installContext

          processInstallPlan args installContext installPlan
          liftIO $ writeIORef retVal ReinstalledSomeDeps

  liftIO $ readIORef retVal
    ...
    where
      ...
      topHandler' x = do
        ...
        flip (topHandlerWithM (flip runCabalM verbosity)) x $ \_ -> do
          warn "Couldn't reinstall some add-source dependencies."
...
withSandboxPackageInfo configFlags globalFlags
  ...
  installedPkgIndex <- getInstalledPackagesInSandbox 
  ...
  depsPkgDescs   <- mapM readGenericPackageDescription depsCabalFiles
  ...
  modifiedAddSourceDeps <- listModifiedDeps sandboxDir
  ...
  if (null modifiedDeps)
    then info   $ "Found no modified add-source deps."
    else notice $ "Some add-source dependencies have been modified. "
...
maybeWithSandboxPackageInfo :: ConfigFlags -> GlobalFlags
                               -> Compiler -> Platform -> ProgramDb
                               -> UseSandbox
                               -> (Maybe SandboxPackageInfo -> CabalM ())
                               -> CabalM ()
maybeWithSandboxPackageInfo configFlags globalFlags
                            comp platform progdb useSandbox cont =
  case useSandbox of
    NoSandbox             -> cont Nothing
    UseSandbox sandboxDir -> withSandboxPackageInfo
...
-- case.
maybeReinstallAddSourceDeps ::    Flag (Maybe Int) -- ^ The '-j' flag
                               -> ConfigFlags      -- ^ Saved configure flags
                                                   -- (from dist/setup-config)
                               -> GlobalFlags
                               -> (UseSandbox, SavedConfig)
                               -> CabalM WereDepsReinstalled
maybeReinstallAddSourceDeps numJobsFlag configFlags'
