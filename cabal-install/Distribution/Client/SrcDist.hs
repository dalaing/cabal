...
import Distribution.Verbosity (normal, lessVerbose)
import Distribution.Monad (CabalM, liftIO, runCabalM, runCabalMInIO, askVerbosity)
...
sdist flags exflags = flip runCabalM verbosity $ do
  pkg <- liftM flattenPackageDescription
    (readGenericPackageDescription =<< defaultPackageDesc)
  let withDir :: (FilePath -> CabalM a) -> CabalM a
      withDir = if not needMakeArchive then \f -> f tmpTargetDir
                else withTempDirectory tmpTargetDir "sdist."
  ...
  withDir $ \tmpDir -> do
    ...
    unless isListSources $
      createDirectoryIfMissingVerbose True outDir

    -- Run 'setup sdist --output-directory=tmpDir' (or
    -- '--list-source'/'--output-directory=someOtherDir') in case we were passed
    -- those options.
    setupWrapper setupOpts (Just pkg) sdistCommand (\_ _ -> flags') []

    -- Unless we were given --list-sources or --output-directory ourselves,
    -- create an archive.
    when needMakeArchive $
      createArchive pkg tmpDir distPref

    when isOutDirectory $
      notice $ "Source directory created: " ++ tmpTargetDir

    when isListSources $
      notice $ "List of package sources written to file '"
...
createTarGzArchive :: PackageDescription -> FilePath -> FilePath
                    -> CabalM ()
createTarGzArchive pkg tmpDir targetPref = do
    liftIO $ createTarGzFile tarBallFilePath tmpDir (tarBallName pkg)
    notice $ "Source tarball created: " ++ tarBallFilePath
  where
    tarBallFilePath = targetPref </> tarBallName pkg <.> "tar.gz"

-- | Create a zip archive from a tree of source files.
createZipArchive :: PackageDescription -> FilePath -> FilePath
                    -> CabalM ()
createZipArchive pkg tmpDir targetPref = do
    let dir       = tarBallName pkg
        zipfile   = targetPref </> dir <.> "zip"
    (zipProg, _) <- requireProgram zipProgram emptyProgramDb
    ...
    notice $ "Source zip archive created: " ++ zipfile
  where
...
allPackageSourceFiles :: SetupScriptOptions -> FilePath
                         -> CabalM [FilePath]
allPackageSourceFiles setupOpts0 packageDir = do
  pkg <- do
    let err = "Error reading source files of package."
    desc <- tryFindAddSourcePackageDesc packageDir err
    flattenPackageDescription `fmap` readGenericPackageDescription desc
  globalTmp <- liftIO getTemporaryDirectory
  withTempDirectory globalTmp "cabal-list-sources." $ \tempDir -> do
  ...
  let file      = tempDir </> "cabal-sdist-list-sources"
      ...
      doListSources :: CabalM [FilePath]
      doListSources = do
        setupWrapper setupOpts (Just pkg) sdistCommand (\_ _ -> flags) []
        liftIO . fmap lines . readFile $ file

      onFailedListSources :: IOException -> CabalM ()
  ...
  r <- runCabalMInIO $ \liftC -> liftC doListSources `catchIO` (\e -> liftC (onFailedListSources e) >> return [])
  -- Ensure that we've closed the 'readFile' handle before we exit the
  -- temporary directory.
  _ <- liftIO $ evaluate (length r)
  return r
